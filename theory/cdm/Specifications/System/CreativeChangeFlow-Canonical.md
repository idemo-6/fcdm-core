...
6. Для креативного режима обязателен safety-hook:
   - `max_decide_delay` (верхняя граница задержки коллапса);
   - `forced_collapse_trigger` (условие принудительного перехода к `decide`/`MetaChangeFlow`).
...
Но наличие safety-hook как класса механизма является обязательным.
